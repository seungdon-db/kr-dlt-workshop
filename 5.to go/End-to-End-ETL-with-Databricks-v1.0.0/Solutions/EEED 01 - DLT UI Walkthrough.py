...
# MAGIC 
# MAGIC Before we get started, we need to seed our to-be-created pipeline with some initial data.
# MAGIC 
# MAGIC The following helper function will land a new set of JSON files in the directory configured as the data source below.
# MAGIC <br/><br/>
# MAGIC 
# MAGIC <img src="https://files.training.databricks.com/images/icon_note_32.png"/>
# MAGIC Note: This functionality has also been duplicated in the notebook <a href="$./EEED 99 - Land New Data" target="_blank">EEED 99 - Land New Data</a>
...
# MAGIC %md ## Generate Pipline Configuration
# MAGIC The configuration of your pipline includes parameters unique to a given user.
# MAGIC 
# MAGIC You will need to specify which language to use by uncommenting the appropriate line.
# MAGIC 
# MAGIC Run the following cell to print out the values used to configure your pipline in subsequent steps.
...
# MAGIC 
# MAGIC We will later add Notebooks #2 & #3 to the pipline but for now, let's focus on just Notebook #1:
# MAGIC 
...
# MAGIC  1. Configure **`APPLY CHANGES INTO`** (at this time, the **`APPLY CHANGES INTO`** functionality shown in this course is still in preview and needs to be enabled here)
# MAGIC     1. Click the **`Add configuration`** button
# MAGIC     1. In the field **Key**, enter the word "**pipelines.applyChangesPreviewEnabled**"
# MAGIC     1. In the field **Value**, enter the word "**true**"
# MAGIC 1. In the field **Target**, enter the value specified in the cell above.
# MAGIC     * This field is optional; if not specified, then tables will not be registered to a metastore, but will still be available in the Databricks File System (DBFS). Refer to the <a href="https://docs.databricks.com/data-engineering/delta-live-tables/delta-live-tables-user-guide.html#publish-tables" target="_blank">documentation</a> for more information on this option.
...
# MAGIC 1. Configure the pipeline to run in local-mode cluster.
# MAGIC     * In most deployments, we would adjust the number of workers to accomodate the scale of the pipline.
...
# MAGIC     * In the fields **Workers**, set the value to "**0**" (zero) - worker and driver will use the same VM.
# MAGIC     * Click **Add configuration** and then set the key to **spark.master** and the corresponding value to **local[*]**
...
# MAGIC 
# MAGIC If you get disconnected from your cluster or have some other event where you wish to land more data without deleting things, refer to the <a href="$./EEED 99 - Land New Data" target="_blank">EEED 99 - Land New Data</a> notebook.
